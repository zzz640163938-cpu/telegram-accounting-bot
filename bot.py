...
async def handle_msg(u, c):
    ...
    if m:
        ...
        cur2.execute("INSERT INTO bills (date,
```python
conn.commit(); conn.close()
    ...
    if m:
        ...
        now = datetime.now(); d = get_effective_date()
        conn = sqlite3.connect(DB_PATH); cur2 = conn.cursor()
        cur2.execute("INSERT INTO bills (date,time,type,name,amount,currency,note,operator_id,operator_name,created_at) VALUES (?,?,?,?,?,?,?,?,?,?)", (d, now.strftime("%H:%M:%S"), tp, nm, amt, "U", "", uid, u.message.from_user.username or "u", now.strftime("%Y-%m-%d %H:%M:%S")))
        conn.commit(); conn.close()
