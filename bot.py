...
async def cmd_delete_bill(u, c):
    ...
    await u.message.reply_text(f"✅ 删除成功，共影响 {n} 条数据")

async def handle_msg(u, c):
    uid = u.message.from_user.id
    if not can_operate(uid) or get_config("switch") != "on": return
    t = u.message.text.strip()
    m = re.match(r"^(.+?)?下发(\d+)([A-Za-zU币]?)$", t)
    if m:
        nm, amt, cur = m.group(1) or "默认", m.group(2), m.group(3) or "U"
        now = datetime.now(); d = get_effective_date()
        conn = sqlite3.connect(DB_PATH); cur2 = conn.cursor()
        cur2.execute("INSERT INTO bills (date,time,type,name,amount,currency,note,operator_id,operator_name,created_at) VALUES (?,?,?,?,?,?,?,?,?,?)", (d, now.strftime("%H:%M:%S"), "下发", nm, float(amt), cur, "", uid, u.message.from_user.username or "u", now.strftime("%Y-%m-%d %H:%M:%S")))
        conn.commit(); conn.close()
        await u.message.reply_text(f"✅ 下发\n👤 {nm}\n💵 {amt} {cur}")
        return
    m = re.match(r"^(.+?)?([+-])(\d+)$", t)
    if m:
        nm, sg, amt = m.group(1) or "默认", m.group(2), m.group(3)
        tp = "入款" if sg == "+" else "出款"
        now = datetime.now(); d = get_effective_date()
        conn = sqlite3.connect(DB_PATH); cur2 = conn.cursor()
        cur2.execute("INSERT INTO bills (date,time,type,name,amount,currency,note,operator_id,operator_name,created_at) VALUES (?,?,?,?,?,?,?,?,?,?)", (d, now.strftime("%H:%M:%S"), tp, nm, float(amt), "U", "", uid, u.message.from_user.username or "u", now.strftime("%Y-%m-%d %H:%M:%S")))
        conn.commit(); conn.close()
        await u.message.reply_text(f"{'📥' if sg=='+' else '📤'} {tp}\n👤 {nm}\n💵 {sg}{amt} U")

def main():
    init_db()
    app = Application.builder().token(BOT_TOKEN).build()
    for h in [
        CommandHandler("help", cmd_help), CommandHandler("start", cmd_help), CommandHandler("配置", cmd_config),
        CommandHandler("开关状态", cmd_switch_status), CommandHandler("到期时间", cmd_expiry),
        CommandHandler("开始", cmd_start), CommandHandler("结束", cmd_stop),
        CommandHandler("设置费率", cmd_set_rate), CommandHandler("设置汇率", cmd_set_exchange_rate),
        CommandHandler("设置日切时间", cmd_day_cut_time), CommandHandler("查看日切时间", cmd_day_cut_time),
        CommandHandler("设置操作人", cmd_set_operator), CommandHandler("移除操作人", cmd_remove_operator),
        CommandHandler("设置管理员", cmd_set_admin), CommandHandler("移除管理员", cmd_remove_admin),
        CommandHandler("机器人名字", cmd_set_bot_name), CommandHandler("今日账单", cmd_today_bill),
        CommandHandler("今日成员账单", cmd_member_bill), CommandHandler("账单明细", cmd_bill_detail),
        CommandHandler("删除账单", cmd_delete_bill),
    ]: app.add_handler(h)
