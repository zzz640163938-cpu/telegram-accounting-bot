...
async def cmd_set_exchange(update: Update, ctx):
    if not is_admin(update.effective_user.id):
        ...
        return
    if not ctx.args:
        await update.message.reply_text("格式：/设置汇率7.4")
        return
...
async def cmd_set_cut(update: Update, ctx):
    ...
    if not ctx.args or not ctx.args[0].isdigit():
        await update.message.reply_text("格式：/设置日切时间14")
        return
...
async def msg_handler(update: Update, ctx):
    ...
    if txt=="今日账单":
        ...
        with open("today_bill.txt","w",encoding="utf-8") as f:f.write(cont)
    ...
    if m_mem:
        ...
        if not res:
            ...
            return
        t=""
        for i in res:t+=f"{i[7]} {i[3]} {i[4]}{i[5]}\n"
    ...
    if m_sub:
        ...
        conn.close()
        text=f"===={tar_date}账单====\n"
        for i in data:text+=f"{i[7]} {i[3]} {i[4]}{i[5]}\n"
        with open(f"bill_{tar_date}.txt","w",encoding="utf-8") as f:f.write(text)
    ...
    if m_day:
        ...
        conn.close()
        txtout=f"===={fd}账单====\n"
        for i in data:txtout+=f"{i[7]} {i[3]} {i[4]}{i[5]}\n"
        with open(f"bill_{fd}.txt","w",encoding="utf-8") as f:f.write(txtout)
    ...
    if m_range:
        ...
        conn.close()
        out=f"===={d1}至{d2}账单====\n"
        for i in data:out+=f"{i[7]} {i[3]} {i[4]}{i[5]}\n"
        with open(f"bill_range_{s1}_{s2}.txt","w",encoding="utf-8") as f:f.write(out)
...
async def main_async():
    ...
    app.add_handler(CommandHandler("设置费率",cmd_set_rate))
    app.add_handler(CommandHandler("设置汇率",cmd_set_exchange))
    app.add_handler(CommandHandler("开始",cmd_switch_start))
    app.add_handler(CommandHandler("结束",cmd_switch_end))
    app.add_handler(CommandHandler("配置",cmd_config))
    app.add_handler(CommandHandler("开关状态",cmd_switch_status))
    app.add_handler(CommandHandler("到期时间",cmd_expire_time))
    app.add_handler(CommandHandler("设置操作人",cmd_set_op))
    app.add_handler(CommandHandler("移除操作人",cmd_del_op))
    app.add_handler(CommandHandler("设置日切时间",cmd_set_cut))
    app.add_handler(CommandHandler("删除账单",cmd_del_today_bill))
    app.add_handler(CommandHandler("设置管理员",cmd_set_admin))
    app.add_handler(CommandHandler("移除管理员",cmd_del_admin))
    app.add_handler(CommandHandler("机器人名字",cmd_rename_bot))
